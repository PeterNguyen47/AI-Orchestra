...
function safeFailureCode(error: unknown): string {
  ...
  return "OPENAI_LIVE_SCRIPT_FAILED";
}

async function main(): Promise<void> {
  if (
    process.env.RUN_LIVE_OPENAI_TESTS !== "true" ||
    process.env.AI_ORCHESTRA_OPENAI_EXECUTION_ENABLED !== "true"
  )
    throw new Error("LIVE_GATE_DISABLED");
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) throw new Error("LIVE_KEY_MISSING");
  const [{ executeGovernedRag }, { OpenAIResponsesAdapter }] = await Promise.all([
    import("../src/server/runtime/executor"),
    import("../src/server/runtime/openai-responses-adapter"),
  ]);
  ...
  writeFileSync(
    "test-results/ao007-openai-live-receipt.json",
    `${JSON.stringify(
      {
        schemaVersion: "1.0.0",

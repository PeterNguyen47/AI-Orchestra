...
## AO-010 adversarial runtime controls

The authenticated action boundary now counts an attempt immediately after session verification. A lower-case SHA-256 subject digest keys a process-local 60-second window: six attempts are allowed and the seventh returns `RATE_LIMIT_EXCEEDED` with 1-through-60-second retry metadata. Raw subjects, cookies, sessions, and digests are never logged. Expired entries are pruned, storage is bounded to 1,024 entries, and a new subject fails closed at capacity. Rate-limited requests create pre-plan RunEvidence and perform zero workflow compilation, adapter construction, provider calls, or other side effects.

The action accepts only `workflow` and `question`. Questions are limited to 8,000 JavaScript characters and workflows to 1,000,000 serialized UTF-8 bytes before runtime configuration is read. Circular values, `BigInt`, undefined serialization, and non-object envelopes fail with `REQUEST_INVALID`. The canonical 4,000-character guardrail still decides runtime input length, so 4,001-through-8,000-character questions reach the guardrail and block with `INPUT_TOO_LONG` without generation.

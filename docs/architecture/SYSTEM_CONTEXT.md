...
AO-011 portable judge mode adds no external system. The browser receives only a safe execution-mode enum and bounded limits. The trusted server selects an in-process deterministic `test_only` adapter, while short-lived non-root Compose tooling manages a removable local credential volume and performs internal fixed-code readiness. Native loopback Ollama remains a separately configured optional live external process. OpenAI stays disabled; no provider, credential, endpoint, host, path, Docker socket, telemetry, database, upload, or connector is exposed to the browser.

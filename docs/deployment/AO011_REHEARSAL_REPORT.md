...
The original implementation-template marker `PENDING_LOCAL_REHEARSAL` is superseded by exact-ref hosted evidence. GitHub Actions run `29872186329` used an ephemeral clean checkout and passed quality, Docker Compose, and browser jobs at the remediation commit below. This is hosted Linux evidence, not a local-workstation rehearsal.

## Rehearsed source

- Remediation commit SHA: `f983cea90d19bf2a7ac8291a0aee159446d74ab3`
- Hosted workflow run: `29872186329` (`pull_request`, exact remediation head)
...
| Rounded available memory  | 15 GiB                                                     |
...
| Tooling and app build            | 49 s     | PASS               |
...
| Interactive AO-011 browser path  | 7.8 s    | PASS               |
| App restart and second readiness | 8 s      | `AO011_READY`      |
| Full scoped teardown             | 1 s      | PASS               |

Aggregate hosted job durations were 62 seconds for Quality and security, 77 seconds for Docker Compose smoke, and 112 seconds for the browser job. The browser test step itself was 58 seconds and is not presented as interactive-path time.
...
| App startup parsed exactly three canonical credential keys without shell evaluation                        | PASS                             |
| Unknown, duplicate, reordered, malformed, and shell-shaped credential lines failed closed                  | PASS                             |
| Forced rotation discarded stale unknown content and wrote the completion marker last                       | PASS                             |
| First internal readiness returned `AO011_READY`                                                            | PASS                             |
| Readiness rejected noncanonical credentials and off-origin or decorated login redirects                    | PASS                             |
...
| A question-supplied chunk marker could not control the retrieved-section citation                          | PASS                             |
| AO-011 trace, screenshot, video, HTML, and temporary browser output retained or uploaded                   | 0                                |
...
The quality job passed 54 Vitest files and 381 cases with 95.11% statements, 90.88% branches, 97.39% functions, and 96.16% lines. The browser job passed ten Chromium scenarios split seven provider-disabled, two legacy governed, and one AO-011 fixture scenario. Legacy fixture counts remained tags=1/version=1/chat=1/unexpected=0.
...
The report records only public commit/run identifiers, platform category, architecture, Docker/Compose versions, rounded resources, cache category, durations, fixed codes, stable test counts, and pass/fail results. Bounded log review found no unmasked authentication value, password hash, session secret, AO-011 temporary path, provider invocation, or model download. The single uploaded browser artifact contained six files; inspection found zero AO-011 relative-path matches and zero AO-011 or credential-content matches, then the temporary inspection copy was deleted. No AO-011 trace, screenshot, video, HTML report, or temporary result was retained or uploaded. Masked checkout authorization records, raw logs, runner paths, loopback bindings, and Docker identifiers were not transferred into this evidence. The AO-007 receipt was absent from the clean checkout, neither accessed nor copied, and does not validate this fixture. No native/containerized Ollama or OpenAI request occurred, so approved OpenAI credits were not used by the rehearsal.

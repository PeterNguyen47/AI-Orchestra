...
AO-011 targets five new Vitest files and 39 added cases for exactly 53 files / 370 cases. New suites cover joint credential bootstrap/marker behavior, fixed-code readiness, runbook drift and prohibited commands, the no-I/O judge adapter, and pure UI truthfulness. Existing runtime-config, model-target, executor, and server-action suites add exactly nine cases for explicit modes, legacy compatibility, adapter selection, test-only evidence, zero-cost deployment handling, one-call success, and zero-call input blocking. Coverage adds the two tooling scripts and judge adapter while retaining the 90/95/95/95 thresholds.
...
Hosted CI retains exactly three jobs. Quality runs locked install, format, lint, typecheck, schema drift, coverage, secret scan, audit, and build. Container CI builds app/tooling from a clean checkout, bootstraps quietly, waits for health, requires `AO011_READY`, restarts app, rechecks readiness, and removes the scoped volume/resources. Browser CI runs all ten scenarios. The separate local clean-clone rehearsal records sanitized environment/timing only; it never reads the AO-007 receipt or invokes a provider.

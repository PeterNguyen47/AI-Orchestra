...
## AO-010 adversarial assurance coverage

AO-010 adds seven Vitest files and 40 cases for a final 48 files and 331 cases. New suites cover the shared sensitive-text detector, strict action request limits, the process-local rate limiter, the security-assurance schema and completion gate, the allowlisted health response, import-safe secret scanning, and the extracted loopback fixture. Existing suites gain fixed input-code, canary, zero-call, upload-readiness, workflow-finding projection, RunEvidence correlation, log-redaction, action, executor, recorder, and adapter assertions without changing RunEvidence `1.0.0` or provider production adapters.

The Chromium suite adds one provider-disabled `@ao010` scenario. Seven baseline scenarios cover health allowlisting, missing/tampered authentication, absent upload and connector APIs, isolated browser state, logout denial, absent file/live-connector controls, and axe. The governed phase remains `@ao008` plus `@ao009`; AO-008 still performs the only fixture chat and AO-009 still performs no generation. Expected total is nine scenarios with fixture counts tags `1`, version `1`, chat `1`, unexpected `0`.

Adversarial inputs are harmless synthetic fragments. Assertions require fixed codes, no raw payload or match disclosure, zero provider calls on pre-execution blocks, no partial exports, no sensitive DOM or log text, safe errors, and bounded process cleanup. The secret scanner reports only relative path and detector name. Coverage includes action, health, security-domain/server modules, scanner, and fixture code under unchanged thresholds.

These tests are deterministic automated assurance for the MVP judge path. They are not live-provider evidence, a penetration test, certification, security approval, production-readiness evidence, or proof that all semantic attacks are detected. The ignored AO-007 receipt remains separate earlier local-provider evidence and is neither read nor regenerated by AO-010.

## AO-011 portable deployment coverage

AO-011 targets six new Vitest files and 50 added cases for exactly 54 files / 381 cases after review remediation. New suites cover joint credential bootstrap/marker behavior, a non-evaluating standalone startup parser, fixed-code readiness, runbook and browser-artifact boundaries, the no-I/O judge adapter, and pure UI truthfulness. Existing runtime-config, model-target, executor, server-action, bootstrap, readiness, runbook, and adapter suites cover explicit modes, legacy compatibility, strict credential parsing and rotation, same-origin redirects, adapter selection, test-only evidence, zero-cost deployment handling, one-call success, zero-call input blocking, and retrieved-section citation selection. The startup parser is exercised by its four-case suite, while the existing explicit coverage include list and 90/95/95/95 thresholds remain unchanged.

The Chromium runner keeps seven provider-disabled scenarios and two legacy `@ao008`/`@ao009` loopback-fixture scenarios, then starts a fresh third app process for one `@ao011` scenario. The legacy fixture remains tags `1`, version `1`, chat `1`, unexpected `0`. AO-011 starts no HTTP fixture, makes one in-process deterministic invocation, and makes zero Ollama/OpenAI requests while testing seed counts, readiness, one governed run, evidence/evaluators/citation, both AO-009 exports, stale restoration without regeneration, one guardrail block, logout, and accessibility.

Hosted CI retains exactly three jobs. Quality runs locked install, format, lint, typecheck, schema drift, coverage, secret scan, audit, and build. Container CI builds app/tooling from an ephemeral GitHub-hosted clean checkout, starts through the non-evaluating credential parser, bootstraps quietly, waits for health, requires `AO011_READY`, restarts app, rechecks readiness, and removes the scoped volume/resources. Browser CI runs all ten scenarios while keeping the credential-sensitive AO-011 phase outside uploaded Playwright artifacts. The hosted clean-checkout rehearsal records only sanitized environment/timing evidence; it never reads the AO-007 receipt or invokes a provider.
